...
public class AWS2S3JsonDataType implements OutputType {
    ...
    public void convertOut(Exchange exchange) {
        ...
        if (bodyInputStream != null) {
            try {
                exchange.getIn().setBody(String.format(JSON_TEMPLATE, key, IoUtils.toUtf8String(bodyInputStream)));
                return;
            } catch (IOException e) {
        ...
        if (bodyContent != null) {
            exchange.getIn().setBody(String.format(JSON_TEMPLATE, key, new String(bodyContent, StandardCharsets.UTF_8)));
        }
    }
}
